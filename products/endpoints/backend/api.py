...
class EndpointViewSet(TeamAndOrgViewSetMixin, PydanticModelMixin, viewsets.ModelViewSet):
        ...
        for endpoint in queryset:
            ...
            if endpoint.is_materialized and endpoint.materialized_query:
                result["materialization"] = {
                    ...
                    "sync_frequency": DataWarehouseSavedQuerySerializer().get_sync_frequency(
    ...
    def retrieve(self, request: Request, name=None, *args, **kwargs) -> Response:
        ...
        if endpoint.is_materialized and endpoint.materialized_query:
            result["materialization"] = {
                ...
                "error": endpoint.materialization_error,
                "sync_frequency": DataWarehouseSavedQuerySerializer().get_sync_frequency(endpoint.materialized_query),
            }
        else:
            result["materialization"] = {
                "can_materialize": can_mat,
        ...
        try:
            ...
            if data.is_materialized is not None:
                if data.is_materialized:
                    # Constraint: if is_materialized, sync_frequency is either requested or default
            ...
            # Handle sync_frequency updates for already materialized endpoints
            # Constraint: sync_frequency can only be set/updated if endpoint.is_materialized
            elif data.sync_frequency is not None:
                if not endpoint.is_materialized:
                    # Constraint: if not is_materialized, sync_frequency is not accepted
                    raise ValidationError("Cannot set sync_frequency on non-materialized endpoint")

                if endpoint.materialized_query:
                    self._update_saved_query_sync_frequency(endpoint.materialized_query, data.sync_frequency, request)
    ...
    def _get_saved_query_context(self, request: Request) -> dict:
        """Create context dict for DataWarehouseSavedQuerySerializer."""
    ...
    def _get_or_create_saved_query(self, endpoint: Endpoint, request: Request) -> DataWarehouseSavedQuery:
        """Get or create a saved query for an endpoint."""
        saved_query_name = f"__endpoint_{self.team.id}_{endpoint.name}"
        saved_query = DataWarehouseSavedQuery.objects.filter(team_id=self.team_id, name=saved_query_name).first()

        if not saved_query:
            context = self._get_saved_query_context(request)
    ...
    def _update_saved_query_sync_frequency(
        self,
        saved_query: DataWarehouseSavedQuery,
        sync_frequency: DataWarehouseSyncInterval,
        request: Request,
    ) -> DataWarehouseSavedQuery:
        """Update sync frequency on a saved query."""
        context = self._get_saved_query_context(request)
        request.data["sync_frequency"] = sync_frequency
    ...
    def _enable_materialization(
        ...
        saved_query = self._get_or_create_saved_query(endpoint, request)
        saved_query = self._update_saved_query_sync_frequency(saved_query, sync_frequency, request)
    ...
    def _cleanup_saved_query(self, saved_query: DataWarehouseSavedQuery) -> None:
        """
        Clean up a saved query including schedule, joins, table, and soft delete.
        Replicates logic from DataWarehouseSavedQueryViewSet.destroy.
        """
    ...
    def _disable_materialization(self, endpoint: Endpoint) -> None:
        ...
        if endpoint.materialized_query:
            self._cleanup_saved_query(endpoint.materialized_query)
